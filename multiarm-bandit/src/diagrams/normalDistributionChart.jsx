...
export default function NormalDistributionChart({ strategies, counts, sumRewards, sigma }) {
  // Daten für jede Strategie vorbereiten
  const chartsData = strategies.map((s, i) => {
  ...
  return (
    <div style={{ marginTop: "2rem" }}>
      <h4>Normalverteilungen der Strategien</h4>
      <LineChart width={700} height={350} data={mergedData}>
        <CartesianGrid strokeDasharray="3 3" />
        <XAxis dataKey="x" label={{ value: "kW", position: "insideBottomRight", offset: -5 }} />
        <YAxis />
        <Tooltip />
        <Legend />
        {strategies.map((s, i) =>
